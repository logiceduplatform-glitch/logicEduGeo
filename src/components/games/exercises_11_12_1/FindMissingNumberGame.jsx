...
export default function FindMissingNumberGame({ lang = "el", onComplete, difficulty = 3 }) {
  ...
  const sequencesData = {
    el: [
      { id: 1, sequence: [7, 14, "?", 28, 35], missingNumber: 21, options: [18, 21, 24], color: "#EF4444", hint: "x7" },
      { id: 2, sequence: [3, 9, 27, "?", 243], missingNumber: 81, options: [54, 81, 108], color: "#10B981", hint: "x3" },
      { id: 3, sequence: [144, "?", 36, 18, 9], missingNumber: 72, options: [72, 64, 80], color: "#3B82F6", hint: "÷2" },
      { id: 4, sequence: [5, 10, 20, "?", 80], missingNumber: 40, options: [30, 40, 50], color: "#EAB308", hint: "x2" },
      { id: 5, sequence: ["?", 13, 26, 39, 52], missingNumber: 0, options: [0, 6, 7], color: "#EC4899", hint: "+13" },
      { id: 6, sequence: [1000, 900, "?", 700, 600], missingNumber: 800, options: [750, 800, 850], color: "#8B5CF6", hint: "-100" },
      { id: 7, sequence: [11, 22, "?", 44, 55], missingNumber: 33, options: [30, 33, 36], color: "#F59E0B", hint: "+11" },
      { id: 8, sequence: [2, 3, 5, 7, "?"], missingNumber: 11, options: [9, 11, 13], color: "#14B8A6", hint: "prime" },
      { id: 9, sequence: [256, 128, 64, "?", 16], missingNumber: 32, options: [32, 36, 48], color: "#06B6D4", hint: "÷2" },
      { id: 10, sequence: [1, 4, 9, "?", 25], missingNumber: 16, options: [14, 16, 20], color: "#A855F7", hint: "n²" },
    ],
    en: [
      { id: 1, sequence: [7, 14, "?", 28, 35], missingNumber: 21, options: [18, 21, 24], color: "#EF4444", hint: "x7" },
      { id: 2, sequence: [3, 9, 27, "?", 243], missingNumber: 81, options: [54, 81, 108], color: "#10B981", hint: "x3" },
  ...
  const speakQuestion = () => {
    if ('speechSynthesis' in window) {
      ...
      const text = lang === 'el' ? "Ποιος αριθμός λείπει από την ακολουθία;" : "Which number is missing from the sequence?";
  ...
  const handleAnswerSelect = (answer) => {
    ...
    if (isCorrect) {
      ...
      updateProgress({ title: "Find Missing Number Game", score: newScore, total: TARGET_ROUNDS, index: currentRound + 1 });
      completeQuiz({ title: "Find Missing Number Game", score: 1, total: 1 });
  ...
  return (
    <div className="relative w-full min-h-screen bg-gradient-to-br from-slate-100 via-blue-100 to-indigo-100 p-4 sm:p-8 rounded-xl overflow-hidden">
      ...
      <div className="bg-white/90 backdrop-blur rounded-xl shadow-lg p-6 mb-6">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h3 className="text-2xl font-bold text-slate-800">
              {lang === "el" ? "Βρες τον Αριθμό - Προχωρημένο" : "Find the Number - Advanced"}
            </h3>
            <p className="text-sm text-slate-600 mt-1">
              {lang === "el" ? `Ακολουθία ${currentRound + 1}/${TARGET_ROUNDS}` : `Sequence ${currentRound + 1}/${TARGET_ROUNDS}`}
            </p>
          </div>
        ...
        <div className="flex items-center gap-3">
          <div className="flex-1 h-4 bg-slate-200 rounded-full overflow-hidden">
            <div className="h-full bg-gradient-to-r from-blue-400 to-indigo-500 transition-all duration-500 ease-out" style={{ width: `${progressPercent}%` }} />
          </div>
      ...
      <div className="text-center mb-8">
        <div className="inline-block bg-white/90 backdrop-blur rounded-2xl p-6 shadow-xl border-4" style={{ borderColor: round.color }}>
          ...
          <button onClick={speakQuestion} className="px-6 py-3 bg-gradient-to-r from-blue-500 to-indigo-500 text-white text-xl font-bold rounded-full hover:scale-105 transition-transform shadow-lg">
        ...
        <div className="flex justify-center gap-3 mb-4 flex-wrap">
          {round.sequence.map((num, index) => (
            <div
              key={index}
              className={`
                w-24 h-24 flex items-center justify-center text-3xl sm:text-4xl font-bold rounded-2xl border-4 transition-all
            ...
            return (
              ...
              >
                <div className="text-center">
                  <div className="text-4xl font-bold text-slate-800">{option}</div>
          ...
          {selectedAnswer === round.missingNumber ? (
            <div className="bg-green-100 rounded-2xl p-6 inline-block border-4 border-green-400">
              <p className="text-3xl font-bold text-green-700">
                {lang === "el" ? `🎉 Σωστά! Ο αριθμός είναι ${round.missingNumber}!` : `🎉 Correct! The number is ${round.missingNumber}!`}
              </p>
            </div>
          ) : (
            <div className="bg-red-100 rounded-2xl p-6 inline-block border-4 border-red-400">
              <p className="text-3xl font-bold text-red-700">
                {lang === "el" ? `Ο σωστός αριθμός είναι ${round.missingNumber}!` : `The correct number is ${round.missingNumber}!`}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-blue-300/80 to-indigo-400/80 flex items-center justify-center rounded-xl z-30 animate-fadeIn">
          <div className="text-center animate-bounce">
            ...
            <h3 className="text-4xl font-bold text-white">
              {lang === "el" ? "Εξαιρετικά! Κατέκτησες τις ακολουθίες!" : "Excellent! You mastered the sequences!"}
            </h3>
          </div>
        </div>
      )}
