...
export default function FindTheSumPairGame({ lang = "el", onComplete }) {
  ...
  const TARGET_ROUNDS = 54;

  const generateProblems = () => {
    const problems = [];
    const base = 70;

    for (let num1 = 2 + base; num1 <= 10 + base; num1++) {
      for (let num2 = 1; num2 <= num1 - base; num2++) {
        const correctAnswer = num1 + num2;

        const wrongAnswers = new Set();
        while (wrongAnswers.size < 2) {
          const wrong = correctAnswer + (Math.floor(Math.random() * 17) - 8);
          if (wrong !== correctAnswer && wrong > 0 && wrong <= 2500) {
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            <h3 className="text-2xl font-bold text-slate-800">
              {lang === "el" ? "Βρες το Σωστό Άθροισμα" : "Find the Right Sum"}
            </h3>
          ...
          <div className="text-center mb-8">
            ...
            <div className="flex justify-center mb-6">
              <div className="inline-block bg-gradient-to-r from-yellow-100 to-orange-100 rounded-3xl p-8 sm:p-12 border-4 border-orange-300 shadow-lg max-w-full overflow-x-auto">
                <div className="flex flex-wrap items-center justify-center gap-6 sm:gap-8 text-6xl sm:text-8xl font-bold text-slate-800">
            ...
            <div className="text-center animate-fadeIn">
              <div className="inline-block bg-green-100 rounded-3xl p-8 border-4 border-green-400">
                ...
                <p className="text-2xl sm:text-3xl font-bold text-green-700">
                  {lang === "el"
                    ? `🎉 Μπράβο! ${currentProblem.num1} + ${currentProblem.num2} = ${currentProblem.correctAnswer}`
                    : `🎉 Great! ${currentProblem.num1} + ${currentProblem.num2} = ${currentProblem.correctAnswer}`}
                </p>
              </div>
            </div>
          )}
            ...
            <div className="text-center animate-fadeIn">
              <div className="bg-red-100 rounded-2xl p-6 inline-block border-4 border-red-400">
                ...
                <p className="text-xl text-red-600 mt-2">
                  {lang === "el"
                    ? `Πρόσθεσε προσεκτικά τα δύο μεγάλα αριθμητικά.`
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-yellow-300/80 to-orange-400/80 flex items-center justify-center rounded-xl z-30 animate-fadeIn">
          <div className="text-center animate-bounce">
            ...
            <h3 className="text-4xl font-bold text-white">
              {lang === "el" ? "Τέλεια! Είσαι άσος στα μεγάλα αθροίσματα!" : "Perfect! You're an addition star!"}
            </h3>
          </div>
        </div>
      )}
