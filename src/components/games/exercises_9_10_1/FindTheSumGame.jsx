...
export default function FindTheSumGame({ lang = "el", onComplete }) {
  ...
  const generateProblems = () => {
    ...
    for (let i = 0; i < TARGET_ROUNDS; i++) {
      ...
      while (wrongAnswers.size < 2) {
        ...
        const wrong = correctAnswer + (off === 0 ? 7 : off);
        if (wrong !== correctAnswer && wrong >= 0 && wrong <= 9999) {
  ...
  const opSymbol = currentProblem.op === "−" ? "−" : currentProblem.op === "×" ? "×" : "+";
  ...
  const exprLine = `${currentProblem.num1} ${opSymbol} ${currentProblem.num2} =`;
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            <h3 className="text-2xl font-bold text-slate-800">
              {lang === "el" ? "Βρες το Σωστό Αποτέλεσμα" : "Find the Correct Result"}
            </h3>
        ...
        <div className="bg-white/90 backdrop-blur rounded-3xl p-8 shadow-xl border-4 border-orange-400">
          <div className="text-center mb-8">
            ...
            <div className="flex justify-center mb-6">
              <div className="inline-block bg-gradient-to-r from-yellow-100 to-orange-100 rounded-3xl p-8 sm:p-12 border-4 border-orange-300 shadow-lg max-w-full overflow-x-auto">
                <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-8 text-5xl sm:text-7xl font-bold text-slate-800">
                  <span className="text-blue-600">{currentProblem.num1}</span>
                  <span className="text-5xl sm:text-6xl text-orange-600">{opSymbol}</span>
                  <span className="text-purple-600">{currentProblem.num2}</span>
                  <span className="text-5xl sm:text-6xl text-orange-600">=</span>
            ...
            <div className="grid grid-cols-3 gap-4 max-w-2xl mx-auto">
              {currentProblem.options.map((option, idx) => (
                <button
                  ...
                  className="p-6 sm:p-8 rounded-2xl bg-gradient-to-br from-yellow-100 to-orange-100 hover:from-yellow-200 hover:to-orange-200 border-4 border-orange-300 hover:border-orange-500 transition-all duration-200 transform hover:scale-105 shadow-lg text-3xl sm:text-5xl font-bold text-slate-800"
                >
                  {option}
                </button>
              ))}
            ...
            <div className="text-center animate-fadeIn">
              <div className="inline-block bg-green-100 rounded-3xl p-8 border-4 border-green-400">
                ...
                <p className="text-2xl sm:text-3xl font-bold text-green-700">
                  {lang === "el"
                    ? `🎉 Μπράβο! ${exprLine} ${currentProblem.correctAnswer}`
                    : `🎉 Great! ${exprLine} ${currentProblem.correctAnswer}`}
                </p>
              </div>
            </div>
          )}
            ...
            <div className="text-center animate-fadeIn">
              <div className="bg-red-100 rounded-2xl p-6 inline-block border-4 border-red-400">
                ...
                <p className="text-xl text-red-600 mt-2">
                  {lang === "el"
                    ? `Υπόλοιπο πράξης: πρόσεξε πρόσθεση, αφαίρεση ή πολλαπλασιασμό.`
                    : `Check whether you need to add, subtract, or multiply.`}
                </p>
              </div>
            </div>
          )}
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-yellow-300/80 to-orange-400/80 flex items-center justify-center rounded-xl z-30 animate-fadeIn">
          <div className="text-center animate-bounce">
            ...
            <h3 className="text-4xl font-bold text-white">
              {lang === "el" ? "Τέλεια! Τα καταφέρνεις στις πράξεις!" : "Perfect! You're a math star!"}
            </h3>
          </div>
        </div>
      )}
