// src/components/games/exercises_6_1/FindTheSumPairGame.jsx
  ...
  const generateProblems = () => {
    ...
    for (let num1 = 2; num1 <= 10; num1++) {
      for (let num2 = 1; num2 <= num1; num2++) {
        ...
        // Δημιουργούμε δύο λάθος απαντήσεις
        const wrongAnswers = new Set();
        while (wrongAnswers.size < 2) {
          const wrong = correctAnswer + (Math.floor(Math.random() * 5) - 2);
          if (wrong !== correctAnswer && wrong > 0 && wrong <= 20) {
            ...
            <div className="text-center animate-fadeIn">
              <div className="bg-red-100 rounded-2xl p-6 inline-block border-4 border-red-400">
                ...
                <p className="text-xl text-red-600 mt-2">
                  {lang === "el"
                    ? `Σκέψου πόσα είναι ${currentProblem.num1} και ${currentProblem.num2} μαζί!`
                    : `Think how many are ${currentProblem.num1} and ${currentProblem.num2} together!`}
                </p>
              </div>
            </div>
          )}
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-yellow-300/80 to-orange-400/80 flex items-center justify-center rounded-xl z-30 animate-fadeIn">
          <div className="text-center animate-bounce">
            ...
            <h3 className="text-4xl font-bold text-white">
              {lang === "el" ? "Τέλεια! Είσαι άσος στις προσθέσεις!" : "Perfect! You're an addition star!"}
            </h3>
          </div>
        </div>
      )}
